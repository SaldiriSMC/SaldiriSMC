...
import { Recordsssdss } from "../configs/tableConfig";
import AddIcon from '@mui/icons-material/Add';
import { loderTrue,loderFalse } from "../actions/Auth";
import { useFormik } from "formik";
import { createRecordsssdss, deleteRecordsssdss , updateRecordsssdss , getRecordsssdss } from "../actions/Recordsssdss";
...
import RECORDSSSDSSModel from "../sharedComponents/RecordsssdssModel";
export default function RECORDSSSDSS() {
  ...
  const allRollsList = useSelector(
    (state) => state?.getRecordsssdss?.getRecordsssdss?.data
  );

  useEffect(() => {

      dispatch(getRecordsssdss({type:'department',filter:filter}));
  ...
  const { handleChange, handleSubmit, handleBlur,setFieldValue, handleReset, errors, values, touched,   setValues,
    ...
    useFormik({
      ...
      onSubmit: () => {
          dispatch( createRecordsssdss({data:{departmentName:values.designationIdCreate},type:'department'}));
  ...
  const handleDeleteModel = () => {
    dispatch(deleteRecordsssdss({type:'department',id:userDeleteId}));
    setShowDeleteModal(false)
  }
  const handleUpdateModel = () => {
    dispatch(updateRecordsssdss({data:{departmentName:values.designationId},type:'department',id:action}));
  ...
  return (
   ...
   <IconButton sx={{mt:3,ml:1}} onClick={()=>setShowModal(true)} size="medium" style={{backgroundColor:"#0075FF", color:"white",}}>
         ...
         <MUITable
            ...
            column={Recordsssdss}
            list={normalizeTableProgram(Recordsssdss?.results ? Recordsssdss?.results : [])}       
          />
          <Grid
            item
            sx={{
