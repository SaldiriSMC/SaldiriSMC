...
import { Recorddfdsdssssdss } from "../configs/tableConfig";
import AddIcon from '@mui/icons-material/Add';
import { loderTrue,loderFalse } from "../actions/Auth";
import { useFormik } from "formik";
import { createRecorddfdsdssssdss, deleteRecorddfdsdssssdss , updateRecorddfdsdssssdss , getRecorddfdsdssssdss } from "../actions/Recorddfdsdssssdss";
...
import RECORDDFDSDSSSSDSSModel from "../sharedComponents/RecorddfdsdssssdssModel";
export default function RECORDDFDSDSSSSDSS() {
  ...
  const allRollsList = useSelector(
    (state) => state?.getRecorddfdsdssssdss?.getRecorddfdsdssssdss?.data
  );

  useEffect(() => {

      dispatch(getRecorddfdsdssssdss({type:'department',filter:filter}));
  ...
  const { handleChange, handleSubmit, handleBlur,setFieldValue, handleReset, errors, values, touched,   setValues,
    ...
    useFormik({
      ...
      onSubmit: () => {
          dispatch( createRecorddfdsdssssdss({data:{departmentName:values.designationIdCreate},type:'department'}));
  ...
  const handleDeleteModel = () => {
    dispatch(deleteRecorddfdsdssssdss({type:'department',id:userDeleteId}));
    setShowDeleteModal(false)
  }
  const handleUpdateModel = () => {
    dispatch(updateRecorddfdsdssssdss({data:{departmentName:values.designationId},type:'department',id:action}));
  ...
  return (
   ...
   <IconButton sx={{mt:3,ml:1}} onClick={()=>setShowModal(true)} size="medium" style={{backgroundColor:"#0075FF", color:"white",}}>
         ...
         <MUITable
            ...
            column={Recorddfdsdssssdss}
            list={normalizeTableProgram(Recorddfdsdssssdss?.results ? Recorddfdsdssssdss?.results : [])}       
          />
          <Grid
            item
            sx={{
