...
import { Recorddfsssdss } from "../configs/tableConfig";
import AddIcon from '@mui/icons-material/Add';
import { loderTrue,loderFalse } from "../actions/Auth";
import { useFormik } from "formik";
import { createRecorddfsssdss, deleteRecorddfsssdss , updateRecorddfsssdss , getRecorddfsssdss } from "../actions/Recorddfsssdss";
...
import RECORDDFSSSDSSModel from "../sharedComponents/RecorddfsssdssModel";
export default function RECORDDFSSSDSS() {
  ...
  const allRollsList = useSelector(
    (state) => state?.getRecorddfsssdss?.getRecorddfsssdss?.data
  );

  useEffect(() => {

      dispatch(getRecorddfsssdss({type:'department',filter:filter}));
  ...
  const { handleChange, handleSubmit, handleBlur,setFieldValue, handleReset, errors, values, touched,   setValues,
    ...
    useFormik({
      ...
      onSubmit: () => {
          dispatch( createRecorddfsssdss({data:{departmentName:values.designationIdCreate},type:'department'}));
  ...
  const handleDeleteModel = () => {
    dispatch(deleteRecorddfsssdss({type:'department',id:userDeleteId}));
    setShowDeleteModal(false)
  }
  const handleUpdateModel = () => {
    dispatch(updateRecorddfsssdss({data:{departmentName:values.designationId},type:'department',id:action}));
  ...
  return (
   ...
   <IconButton sx={{mt:3,ml:1}} onClick={()=>setShowModal(true)} size="medium" style={{backgroundColor:"#0075FF", color:"white",}}>
         ...
         <MUITable
            ...
            column={Recorddfsssdss}
            list={normalizeTableProgram(Recorddfsssdss?.results ? Recorddfsssdss?.results : [])}       
          />
          <Grid
            item
            sx={{
