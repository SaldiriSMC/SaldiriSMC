...
import { carListlll } from "../configs/tableConfig";
import AddIcon from '@mui/icons-material/Add';
import { loderTrue,loderFalse } from "../actions/Auth";
import { useFormik } from "formik";
import { createcarListlll, deletecarListlll , updatecarListlll , getcarListlll } from "../actions/carListlll";
...
import CARLISTLLLModel from "../sharedComponents/carListlllModel";
export default function CARLISTLLL() {
  ...
  const allRollsList = useSelector(
    (state) => state?.getcarListlll?.getcarListlll?.data
  );

  useEffect(() => {

      dispatch(getcarListlll({type:'department',filter:filter}));
  ...
  const { handleChange, handleSubmit, handleBlur,setFieldValue, handleReset, errors, values, touched,   setValues,
    ...
    useFormik({
      ...
      onSubmit: () => {
          dispatch( createcarListlll({data:{departmentName:values.designationIdCreate},type:'department'}));
  ...
  const handleDeleteModel = () => {
    dispatch(deletecarListlll({type:'department',id:userDeleteId}));
    setShowDeleteModal(false)
  }
  const handleUpdateModel = () => {
    dispatch(updatecarListlll({data:{departmentName:values.designationId},type:'department',id:action}));
  ...
  return (
   ...
   <IconButton sx={{mt:3,ml:1}} onClick={()=>setShowModal(true)} size="medium" style={{backgroundColor:"#0075FF", color:"white",}}>
         ...
         <MUITable
            ...
            column={carListlll}
            list={normalizeTableProgram(carListlll?.results ? carListlll?.results : [])}       
          />
          <Grid
            item
            sx={{
