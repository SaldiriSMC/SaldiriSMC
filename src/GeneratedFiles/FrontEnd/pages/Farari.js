...
import { Farari } from "../configs/tableConfig";
import AddIcon from '@mui/icons-material/Add';
import { loderTrue,loderFalse } from "../actions/Auth";
import { useFormik } from "formik";
import { createFarari, deleteFarari , updateFarari , getFarari } from "../actions/Farari";
...
import FARARIModel from "../sharedComponents/FarariModel";
export default function FARARI() {
  ...
  const allRollsList = useSelector(
    (state) => state?.getFarari?.getFarari?.data
  );

  useEffect(() => {

      dispatch(getFarari({type:'department',filter:filter}));
  ...
  const { handleChange, handleSubmit, handleBlur,setFieldValue, handleReset, errors, values, touched,   setValues,
    ...
    useFormik({
      ...
      onSubmit: () => {
          dispatch( createFarari({data:{departmentName:values.designationIdCreate},type:'department'}));
  ...
  const handleDeleteModel = () => {
    dispatch(deleteFarari({type:'department',id:userDeleteId}));
    setShowDeleteModal(false)
  }
  const handleUpdateModel = () => {
    dispatch(updateFarari({data:{departmentName:values.designationId},type:'department',id:action}));
  ...
  return (
   ...
   <IconButton sx={{mt:3,ml:1}} onClick={()=>setShowModal(true)} size="medium" style={{backgroundColor:"#0075FF", color:"white",}}>
         ...
         <MUITable
            ...
            column={Farari}
            list={normalizeTableProgram(Farari?.results ? Farari?.results : [])}       
          />
          <Grid
            item
            sx={{
