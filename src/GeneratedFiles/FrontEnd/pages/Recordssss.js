...
import { Recordssss } from "../configs/tableConfig";
import AddIcon from '@mui/icons-material/Add';
import { loderTrue,loderFalse } from "../actions/Auth";
import { useFormik } from "formik";
import { createRecordssss, deleteRecordssss , updateRecordssss , getRecordssss } from "../actions/Recordssss";
...
import RECORDSSSSModel from "../sharedComponents/RecordssssModel";
export default function RECORDSSSS() {
  ...
  const allRollsList = useSelector(
    (state) => state?.getRecordssss?.getRecordssss?.data
  );

  useEffect(() => {

      dispatch(getRecordssss({type:'department',filter:filter}));
  ...
  const { handleChange, handleSubmit, handleBlur,setFieldValue, handleReset, errors, values, touched,   setValues,
    ...
    useFormik({
      ...
      onSubmit: () => {
          dispatch( createRecordssss({data:{departmentName:values.designationIdCreate},type:'department'}));
  ...
  const handleDeleteModel = () => {
    dispatch(deleteRecordssss({type:'department',id:userDeleteId}));
    setShowDeleteModal(false)
  }
  const handleUpdateModel = () => {
    dispatch(updateRecordssss({data:{departmentName:values.designationId},type:'department',id:action}));
  ...
  return (
   ...
   <IconButton sx={{mt:3,ml:1}} onClick={()=>setShowModal(true)} size="medium" style={{backgroundColor:"#0075FF", color:"white",}}>
         ...
         <MUITable
            ...
            column={Recordssss}
            list={normalizeTableProgram(Recordssss?.results ? Recordssss?.results : [])}       
          />
          <Grid
            item
            sx={{
