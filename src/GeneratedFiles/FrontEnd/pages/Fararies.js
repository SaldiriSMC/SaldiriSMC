...
import { Fararies } from "../configs/tableConfig";
import AddIcon from '@mui/icons-material/Add';
import { loderTrue,loderFalse } from "../actions/Auth";
import { useFormik } from "formik";
import { createFararies, deleteFararies , updateFararies , getFararies } from "../actions/Fararies";
...
import FARARIESModel from "../sharedComponents/FarariesModel";
export default function FARARIES() {
  ...
  const allRollsList = useSelector(
    (state) => state?.getFararies?.getFararies?.data
  );

  useEffect(() => {

      dispatch(getFararies({type:'department',filter:filter}));
  ...
  const { handleChange, handleSubmit, handleBlur,setFieldValue, handleReset, errors, values, touched,   setValues,
    ...
    useFormik({
      ...
      onSubmit: () => {
          dispatch( createFararies({data:{departmentName:values.designationIdCreate},type:'department'}));
  ...
  const handleDeleteModel = () => {
    dispatch(deleteFararies({type:'department',id:userDeleteId}));
    setShowDeleteModal(false)
  }
  const handleUpdateModel = () => {
    dispatch(updateFararies({data:{departmentName:values.designationId},type:'department',id:action}));
  ...
  return (
   ...
   <IconButton sx={{mt:3,ml:1}} onClick={()=>setShowModal(true)} size="medium" style={{backgroundColor:"#0075FF", color:"white",}}>
         ...
         <MUITable
            ...
            column={Fararies}
            list={normalizeTableProgram(Fararies?.results ? Fararies?.results : [])}       
          />
          <Grid
            item
            sx={{
