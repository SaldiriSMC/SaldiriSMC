...
import { ssss } from "../configs/tableConfig";
import AddIcon from '@mui/icons-material/Add';
import { loderTrue,loderFalse } from "../actions/Auth";
import { useFormik } from "formik";
import { createssss, deletessss , updatessss , getssss } from "../actions/ssss";
...
import SSSSModel from "../sharedComponents/ssssModel";
export default function SSSS() {
  ...
  const allRollsList = useSelector(
    (state) => state?.getssss?.getssss?.data
  );

  useEffect(() => {

      dispatch(getssss({type:'department',filter:filter}));
  ...
  const { handleChange, handleSubmit, handleBlur,setFieldValue, handleReset, errors, values, touched,   setValues,
    ...
    useFormik({
      ...
      onSubmit: () => {
          dispatch( createssss({data:{departmentName:values.designationIdCreate},type:'department'}));
  ...
  const handleDeleteModel = () => {
    dispatch(deletessss({type:'department',id:userDeleteId}));
    setShowDeleteModal(false)
  }
  const handleUpdateModel = () => {
    dispatch(updatessss({data:{departmentName:values.designationId},type:'department',id:action}));
  ...
  return (
   ...
   <IconButton sx={{mt:3,ml:1}} onClick={()=>setShowModal(true)} size="medium" style={{backgroundColor:"#0075FF", color:"white",}}>
         ...
         <MUITable
            ...
            column={ssss}
            list={normalizeTableProgram(ssss?.results ? ssss?.results : [])}       
          />
          <Grid
            item
            sx={{
