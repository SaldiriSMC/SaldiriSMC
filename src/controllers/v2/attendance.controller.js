...
const createAttendance = catchAsync(async (req, res) => {
  ...
  const time = await Time.create({timeIn:`${todayDate} ${req.body.timeIn}`, timeOut:`${todayDate} ${req.body.timeOut}`, attendanceId:attendance.id});
...
const updateAttendance = catchAsync(async (req, res) => {
  const time = await Time.update({timeIn:`${todayDate} ${req.body.timeIn}`, timeOut:`${todayDate} ${req.body.timeOut}`}, { where: { id: req.body.id } });
  updateWorkedHours(req.params.attendanceId)
  if(time[0] === 1){
...
const getAttendanceByHours = catchAsync(async(req, res) =>{
  ...
  const result = callDBRoutine("get_AttendanceByHours", {'id':id, 'date': new Date()}, res)
})
...
module.exports = { getAttendance, updateAttendance, deleteAttendance, getAttendanceWithWorkedHours, getAttendanceByHours, createAttendance};
